water <- read.csv("D:/Workplace/Environmental_Statistics_with_R/논문데이터분석(LASSO)/자료, 결과정리/PCA에 Rain, Flow 포함/csv파일/10_함평.csv", sep=",", header=T)
water1 <- water
...
library(psych)
water <- read.csv("C:/Users/Nier/Desktop/?���?????ͺм?/?���?????ͺм?(A study on evaluate water quality using LASSO in Yeongsan River, Republic of Korea)/?????????ڷ?/??ü??��.csv", header=T, sep=',')
water <- water[,-11]
water <- water[,-14]
water <- water[,-(18:19)]
water_scale <- scale(water)
