...
sqrt(sum((data.mlr$response-data.mlr$fitted_values)^2)/length(data.mlr$response)) # RMSE = 18872.23   
...
par(mfrow=c(1,1))
plot(data.mlr$time,data.mlr$response,type='o',col='blue',xlab="time",ylab="TC",main="response vs fitted_values (Multiple Linear Regression)")
lines(data.mlr$time,data.mlr$fitted_values,type='o',col='red')
legend(3800,10e+04,c("fitted_values","response"),lwd=c(1,1),col=c("red","blue"))
```
...
#### 최종선택된 모형의 예측력(Root Mean Squared Error) : 18872.23(다중선형회귀모형), 28977.33(일반화선형모형), 24377.83(일반화가법모형), 37901.47(시간에 따른 계수변화모형)
...
```{r}
par(mfrow=c(1,1))
plot(data.mlr$time,data.mlr$fitted_values,type='o',col='red',xlab="time",ylab="TC",main="response vs fitted_values (Multiple Linear Regression)",ylim=c(-50000,200000))
lines(data.mlr$time,data.mlr$response,type='o',col='blue')
legend(3650,200000,c("fitted_values","response"),lwd=c(1,1),col=c("red","blue"))
plot(data.glm$time,data.glm$fitted_values,type='o',col='red',xlab="time",ylab="TC",main="response vs fitted_values (Generalized Linear Model)",ylim=c(-50000,200000))
lines(data.glm$time,data.glm$response,type='o',col='blue')
legend(3650,200000,c("fitted_values","response"),lwd=c(1,1),col=c("red","blue"))
plot(data.gam$time,data.gam$fitted_values,type='o',col='red',xlab="time",ylab="TC",main="response vs fitted_values (Generalized Additive Model)",ylim=c(-50000,200000))
lines(data.gam$time,data.gam$response,type='o',col='blue')
legend(3650,200000,c("fitted_values","response"),lwd=c(1,1),col=c("red","blue"))
plot(data.tvcm$time,data.tvcm$fitted_values,type='o',col='red',xlab="time",ylab="TC",main="response vs fitted_values (Time Varying Coefficient Model)",ylim=c(-50000,200000))
lines(data.tvcm$time,data.tvcm$response,type='o',col='blue')
legend(3650,200000,c("fitted_values","response"),lwd=c(1,1),col=c("red","blue"))
```
