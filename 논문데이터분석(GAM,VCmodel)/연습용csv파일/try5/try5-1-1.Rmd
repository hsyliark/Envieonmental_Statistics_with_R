...
summary(fit) # Deviance explained:     
vif(fit) # 다중공선성(multicollinearity) 존재
par(mfrow=c(2,2))
plot(fit)
par(mfrow=c(1,1))
...
fit.step <- stepAIC(fit, direction="both", trace=TRUE)
fit.step # 선택변수 :  
summary(fit.step) # Deviance explained:      
vif(fit.step) # 다중공선성(multicollinearity) 존재 
par(mfrow=c(2,2))
plot(fit.step)
par(mfrow=c(1,1))
dwtest(Chla~,data=ex1) # 잔차가 양의 자기상관성을 가지고 있음.
# 다중회귀분석에 대한 기본가정인 잔차(residual)에 대한 정규성, 등분산성, 독립성, 선형성 등을 만족하지 못함.
```
...
sqrt(sum((data.mlr1$response-data.mlr1$fitted_values)^2)/length(data.mlr1$response)) # RMSE =    
ggplot(data.mlr1, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
...
sqrt(sum((data.glm1$response-data.glm1$fitted_values)^2)/length(data.glm1$response)) # RMSE =  
ggplot(data.glm1, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
