...
#### 모형의 설명력은 Deviance explained(다양한 형태의 모형을 비교하기 위해 R-square 통계량을 확장한 형태) 통계량 기준으로(산출된 값이 클수록 좋음.), 모형의 예측력은 MSE(Mean Squared Error) 통계량 기준으로 해석함(산출된 값이 작을수록 좋음.). 
...
summary(fit.step) # Deviance explained: 0.6524  
vif(fit.step) 
par(mfrow=c(2,2))
plot(fit.step)
par(mfrow=c(1,1))
...
sum((data.mlr$response-data.mlr$fitted_values)^2)/length(data.mlr$response) # MSE = 508449377  
...
sum((data.glm$response-data.glm$fitted_values)^2)/length(data.glm$response) # MSE = 689792898 
...
mm <- gam(TC~s(pH)+s(DO)+s(BOD)+s(COD)+s(SS)+s(TN)+s(TP)+s(TOC)+s(WT)+s(EC)+s(Chla)+s(NH3N)+s(PO4P)+s(FC)+s(Flow)+s(Rain),data=ex1,family=quasipoisson(link="log"))
...
```{r}
mm.shrink <- gam(TC~s(pH)+s(DO)+s(BOD)+s(COD)+s(SS)+s(TN)+s(TP)+s(TOC)+s(WT)+s(EC)+s(Chla)+s(NH3N)+s(PO4P)+s(FC)+s(Flow)+s(Rain),data=ex1,family=quasipoisson(link="log"),method="GCV.Cp",select=TRUE) 
plot(mm.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm.shrink)[1])
ggGam(mm.shrink) # 선택변수 : pH, DO, COD, SS, TN, TP, TOC, WT, EC, Chla, PO4P, FC, Flow, Rain 
summary(mm.shrink) # Deviance explained = 87.2% 
...
sum((data.gam$response-data.gam$fitted_values)^2)/length(data.gam$response) # MSE = 189284707 
...
par(mar=c(4,4,4,4))
par(oma=c(0.5,0.5,0.5,0.5))
vc <- gam(TC~s(time)+s(time,by=pH)+s(time,by=DO)+s(time,by=BOD)+s(time,by=COD)+s(time,by=SS)+s(time,by=TN)+s(time,by=TP)+s(time,by=TOC)+s(time,by=WT)+s(time,by=EC)+s(time,by=Chla)+s(time,by=NH3N)+s(time,by=PO4P)+s(time,by=FC)+s(time,by=Flow)+s(time,by=Rain),data=ex1,family=quasipoisson(link="log"))
plot(vc,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc)[1],ylim=c(-40,40))
ggGam(vc)
summary(vc) # Deviance explained = 88.9%  
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
```{r}
vc.shrink <- gam(TC~s(time)+s(time,by=pH)+s(time,by=DO)+s(time,by=BOD)+s(time,by=COD)+s(time,by=SS)+s(time,by=TN)+s(time,by=TP)+s(time,by=TOC)+s(time,by=WT)+s(time,by=EC)+s(time,by=Chla)+s(time,by=NH3N)+s(time,by=PO4P)+s(time,by=FC)+s(time,by=Flow)+s(time,by=Rain),data=ex1,family=quasipoisson(link="log"),method="GCV.Cp",select=TRUE)
plot(vc.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc.shrink)[1],ylim=c(-40,40))
ggGam(vc.shrink) # 선택변수 : DO, BOD, COD, SS, TN, TP, TOC, WT, EC, Chla, NH3N, PO4P, FC, Flow, Rain  
...
sum((data.tvcm$response-data.tvcm$fitted_values)^2)/length(data.tvcm$response) # MSE = 153730821 
...
#### 최종선택된 모형의 설명력(Deviance explained) : 65.2%(다중선형회귀모형), 62.1%(일반화선형모형), 87.2%(일반화가법모형), 86.8%(시간에 따른 계수변화모형) 

#### 최종선택된 모형의 예측력(Mean Squared Error) : 508449377(다중선형회귀모형), 689792898(일반화선형모형), 189284707(일반화가법모형), 153730821(시간에 따른 계수변화모형)
...
sum((data.mlr$response-data.mlr$fitted_values)^2)/length(data.mlr$response) # MSE = 54327884
ggplot(data.mlr, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Multiple Linear Regression)")
```
...
sum((data.glm$response-data.glm$fitted_values)^2)/length(data.glm$response) # MSE = 68413583 
ggplot(data.glm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Linear Model)")
```
...
sum((data.gam$response-data.gam$fitted_values)^2)/length(data.gam$response) # MSE = 8020085 
ggplot(data.gam, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Additive Model)")
```
...
sum((data.tvcm$response-data.tvcm$fitted_values)^2)/length(data.tvcm$response) # MSE = 8817523 
ggplot(data.tvcm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Time Varying Coefficient Model)")
```
...
#### 최종선택된 모형의 예측력(Mean Squared Error) : 54327884(다중선형회귀모형), 68413583(일반화선형모형), 8020085(일반화가법모형), 8817523(시간에 따른 계수변화모형)
...
sum((data.mlr$response-data.mlr$fitted_values)^2)/length(data.mlr$response) # MSE = 385.3183 
ggplot(data.mlr, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Multiple Linear Regression)")
```
...
sum((data.glm$response-data.glm$fitted_values)^2)/length(data.glm$response) # MSE = 955.7093
ggplot(data.glm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Linear Model)")
```
...
sum((data.gam$response-data.gam$fitted_values)^2)/length(data.gam$response) # MSE = 260.9037 
ggplot(data.gam, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Additive Model)")
```
...
plot(vc,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc)[1],ylim=c(-20,20))
ggGam(vc)
...
plot(vc.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc.shrink)[1],ylim=c(-20,20))
...
sum((data.tvcm$response-data.tvcm$fitted_values)^2)/length(data.tvcm$response) # MSE = 249.6875 
ggplot(data.tvcm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Time Varying Coefficient Model)")
```
...
#### 최종선택된 모형의 예측력(Mean Squared Error) : 385.3183(다중선형회귀모형), 955.7093(일반화선형모형), 260.9037(일반화가법모형), 249.6875(시간에 따른 계수변화모형)
...
sum((data.mlr$response-data.mlr$fitted_values)^2)/length(data.mlr$response) # MSE = 56031284
ggplot(data.mlr, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Multiple Linear Regression)")
```
...
sum((data.glm$response-data.glm$fitted_values)^2)/length(data.glm$response) # MSE = 66313189
ggplot(data.glm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Linear Model)")
```
...
sum((data.gam$response-data.gam$fitted_values)^2)/length(data.gam$response) # MSE = 31328586
ggplot(data.gam, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Additive Model)")
```
...
sum((data.tvcm$response-data.tvcm$fitted_values)^2)/length(data.tvcm$response) # MSE = 25575507
ggplot(data.tvcm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Time Varying Coefficient Model)")
```
...
#### 최종선택된 모형의 예측력(Mean Squared Error) : 56031284(다중선형회귀모형), 66313189(일반화선형모형), 31328586(일반화가법모형), 25575507(시간에 따른 계수변화모형)
...
sum((data.mlr$response-data.mlr$fitted_values)^2)/length(data.mlr$response) # MSE = 4042476
ggplot(data.mlr, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Multiple Linear Regression)")
```
...
sum((data.glm$response-data.glm$fitted_values)^2)/length(data.glm$response) # MSE = 2196318
ggplot(data.glm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Linear Model)")
```
...
sum((data.gam$response-data.gam$fitted_values)^2)/length(data.gam$response) # MSE = 362007.2
ggplot(data.gam, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Additive Model)")
```
...
sum((data.tvcm$response-data.tvcm$fitted_values)^2)/length(data.tvcm$response) # MSE = 330211
ggplot(data.tvcm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Time Varying Coefficient Model)")
```
...
#### 최종선택된 모형의 예측력(Mean Squared Error) : 4042476(다중선형회귀모형), 2196318(일반화선형모형), 362007.2(일반화가법모형), 330211(시간에 따른 계수변화모형)
...
sum((data.mlr$response-data.mlr$fitted_values)^2)/length(data.mlr$response) # MSE = 159.7184
ggplot(data.mlr, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Multiple Linear Regression)")
```
...
sum((data.glm$response-data.glm$fitted_values)^2)/length(data.glm$response) # MSE = 522.3904
ggplot(data.glm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Linear Model)")
```
...
sum((data.gam$response-data.gam$fitted_values)^2)/length(data.gam$response) # MSE =
ggplot(data.gam, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Additive Model)")
```
...
sum((data.tvcm$response-data.tvcm$fitted_values)^2)/length(data.tvcm$response) # MSE =
ggplot(data.tvcm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Time Varying Coefficient Model)")
```
...
#### 최종선택된 모형의 예측력(Mean Squared Error) : %(다중선형회귀모형), %(일반화선형모형), %(일반화가법모형), %(시간에 따른 계수변화모형)
