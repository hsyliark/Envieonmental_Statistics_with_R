...
sum((data.tvcm$response-data.tvcm$fitted_values)^2)/length(data.tvcm$response) # MSE = 70.10814
ggplot(data.tvcm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Time Varying Coefficient Model)")
```
...
#### 최종선택된 모형의 예측력(Mean Squared Error) : 159.7184(다중선형회귀모형), 522.3904(일반화선형모형), 69.5277(일반화가법모형), 70.10814(시간에 따른 계수변화모형)
