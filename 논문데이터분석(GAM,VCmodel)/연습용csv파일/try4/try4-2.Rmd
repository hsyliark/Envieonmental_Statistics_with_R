...
### 모형적합 시 반응변수의 분포에 대한 당위성을 위하여 TC와 FC의 경우(counting variable)는 잔차의 분포를 포아송분포(Poisson Distribution)로, Chla의 경우(positive real number variable)는 잔차의 분포를 감마분포(Gamma Distribution)로 가정함.
...
fit <- glm(TC~.,data=ex1,family=gaussian(link="identity"))
...
grid.arrange(p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,nrow=4,ncol=4)
fit <- glm(FC~.,data=ex1,family=gaussian(link="identity"))
...
#### 최종선택된 모형의 설명력 : 62.1%(다중선형회귀모형), 69.7%(일반화선형모형), 93.4%(일반화가법모형), 94.6%(시간에 따른 계수변화모형) 
...
grid.arrange(p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,nrow=4,ncol=4)
fit <- glm(Chla~.,data=ex1,family=gaussian(link="identity"))
...
ggGam(vc)
summary(vc) # Deviance explained = 75.4%
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
ggGam(vc.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TN, TP, TOC, WT, EC, TC, NH3N, PO4P, Flow, Rain 
summary(vc.shrink) # Deviance explained = 72.8% 
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
#### 최종선택된 모형의 설명력 : 65.6%(다중선형회귀모형), 60.2%(일반화선형모형), 72.2%(일반화가법모형), 72.8%(시간에 따른 계수변화모형) 
...
gfit <- glm(TC~.,data=ex2,family=gaussian(link="identity"))
...
grid.arrange(g1,g2,g3,g4,g5,g6,g7,g8,g9,g10,g11,g12,g13,g14,g15,g16,nrow=4,ncol=4)
gfit <- glm(FC~.,data=ex2,family=gaussian(link="identity"))
...
#### 최종선택된 모형의 설명력 : 40.6%(다중선형회귀모형), 60.6%(일반화선형모형), 88.7%(일반화가법모형), 90.7%(시간에 따른 계수변화모형)  
...
grid.arrange(p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,nrow=4,ncol=4)
gfit <- glm(Chla~.,data=ex2,family=gaussian(link="identity"))
...
plot(vc,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc)[1])
ggGam(vc)
summary(vc) # Deviance explained = 80.2% 
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
summary(vc.shrink) # Deviance explained =   80% 
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
#### 최종선택된 모형의 설명력 : 71.2%(다중선형회귀모형), 69.0%(일반화선형모형), 79.2%(일반화가법모형), 80.0%(시간에 따른 계수변화모형) 
