...
#### 상관분석이나 모형적합 시 TC와 FC를 반응변수로 두는 경우에는 각 수질항목의 원값을 그대로 사용하여 표현하는 것이 더 적절할 수 있다고 판단하여 표준화를 하지 않았으며, Chla를 반응변수로 두는 경우에는 시스템적인 오류가 발생하는 경우도 종종 있고 표준화 여부에 관계없이 모형의 설명력은 동일하기 때문에 표준화를 실시함.
...
ggplot(data.mlr, aes(x=fitted_values, y=response)) +
  geom_point() + geom_rug() +
...
ggplot(data.glm, aes(x=fitted_values, y=response)) +
  geom_point() + geom_rug() +
...
ggGam(mm)
summary(mm) # R-sq.(adj) =  0.897   Deviance explained = 89.9%
...
summary(mm.shrink) # R-sq.(adj) =  0.897   Deviance explained = 89.8%
...
ggplot(data.gam, aes(x=fitted_values, y=response)) +
  geom_point() + geom_rug() +
...
ggGam(vc)
summary(vc) # R-sq.(adj) =   0.93   Deviance explained = 90.8%
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
summary(vc.shrink) # R-sq.(adj) =  0.931   Deviance explained = 90.7%
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
ggplot(data.tvcm, aes(x=fitted_values, y=response)) +
  geom_point() + geom_rug() +
...
plot(mm,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm)[1])
ggGam(mm)
summary(mm) # R-sq.(adj) =  0.954   Deviance explained = 93.4%
concurvity(mm,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
summary(mm.shrink) # R-sq.(adj) =  0.955   Deviance explained = 93.4%
concurvity(mm.shrink,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm.shrink) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
plot(vc,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc)[1],ylim=c(-50,50))
ggGam(vc)
summary(vc) # R-sq.(adj) =  0.977   Deviance explained = 94.7%
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
summary(vc.shrink) # R-sq.(adj) =  0.976   Deviance explained = 94.6%
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
#### 최종선택된 모형의 설명력 : 61.7%(다중선형회귀모형), 69.7%(일반화선형모형), 93.4%(일반화가법모형), 94.6%(시간에 따른 계수변화모형) 
...
summary(fit.step) # Adjusted R-squared:  0.6484 
...
```{r}
pred.mlr <- predict(fit.step,newdata=ex1_scale,type="response")
data.mlr <- data.frame(response=ex1_scale$Chla,fitted_values=pred.mlr)
ggplot(data.mlr, aes(x=fitted_values, y=response)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Multiple Linear Regression)")
```
...
### Generalized Linear Model
#### Chl-a의 경우는 연속형 변수이므로 일반적인 정규동일모형(normal identity model)을 가정해도 무방함.

```{r}
m <- glm(Chla~pH+DO+BOD+COD+SS+TN+TP+TOC+WT+EC+FC+NH3N+PO4P+TC+Flow+Rain,data=ex1_scale,family=gaussian(link="identity"))
summary(m) # Deviance explained : 0.6581
vif(m) # 다중공선성(multicollinearity) 존재
par(mfrow=c(2,2))
plot(m)
par(mfrow=c(1,1))
...
```{r}
m.step <- stepAIC(m, direction="both", trace=TRUE)
m.step # 선택변수 : pH, DO, BOD, COD, SS, TP, TOC, WT, TC, NH3N, PO4P
summary(m.step) # Deviance explained : 0.6560
vif(m.step) # 다중공선성(multicollinearity) 존재 
par(mfrow=c(2,2))
plot(m.step)
par(mfrow=c(1,1))
...
```{r}
pred.glm <- predict(m.step,newdata=ex1_scale,type="response")
data.glm <- data.frame(response=ex1_scale$Chla,fitted_values=pred.glm)
ggplot(data.glm, aes(x=fitted_values, y=response)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Linear Model)")
```
...
```{r}
# GAM의 경우 추정되는 계수(coefficients)가 데이터의 개수보다 많으면 적합되지 않음.
mm <- gam(Chla~s(pH)+s(DO)+s(BOD)+s(COD)+s(SS)+s(TN)+s(TP)+s(TOC)+s(WT)+s(EC)+s(TC)+s(NH3N)+s(PO4P)+s(FC)+s(Flow)+s(Rain),data=ex1_scale,family=gaussian(link="identity"))
plot(mm,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm)[1])
ggGam(mm)
summary(mm) # R-sq.(adj) =  0.706   Deviance explained = 73.6%
concurvity(mm,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
```{r}
mm.shrink <- gam(Chla~s(pH)+s(DO)+s(BOD)+s(COD)+s(SS)+s(TN)+s(TP)+s(TOC)+s(WT)+s(EC)+s(TC)+s(NH3N)+s(PO4P)+s(FC)+s(Flow)+s(Rain),data=ex1_scale,family=gaussian(link="identity"),method="GCV.Cp",select=TRUE) 
plot(mm.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm.shrink)[1])
ggGam(mm.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TP, WT, EC, TC, NH3N, PO4P, FC, Rain
summary(mm.shrink) # R-sq.(adj) =  0.706   Deviance explained = 73.3%
concurvity(mm.shrink,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm.shrink) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
```{r}
pred.gam <- predict(mm.shrink,newdata=ex1_scale,type="response")
data.gam <- data.frame(response=ex1_scale$Chla,fitted_values=pred.gam)
ggplot(data.gam, aes(x=fitted_values, y=response)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Additive Model)")
```
...
head(ex1_scale)
vc <- gam(Chla~s(time)+s(time,by=pH)+s(time,by=DO)+s(time,by=BOD)+s(time,by=COD)+s(time,by=SS)+s(time,by=TN)+s(time,by=TP)+s(time,by=TOC)+s(time,by=WT)+s(time,by=EC)+s(time,by=TC)+s(time,by=NH3N)+s(time,by=PO4P)+s(time,by=FC)+s(time,by=Flow)+s(time,by=Rain),data=ex1_scale,family=gaussian(link="identity"))
plot(vc,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc)[1])
ggGam(vc)
summary(vc) # R-sq.(adj) =  0.731   Deviance explained = 76.4%
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
```{r}
vc.shrink <- gam(Chla~s(time)+s(time,by=pH)+s(time,by=DO)+s(time,by=BOD)+s(time,by=COD)+s(time,by=SS)+s(time,by=TN)+s(time,by=TP)+s(time,by=TOC)+s(time,by=WT)+s(time,by=EC)+s(time,by=TC)+s(time,by=NH3N)+s(time,by=PO4P)+s(time,by=FC)+s(time,by=Flow)+s(time,by=Rain),data=ex1_scale,family=gaussian(link="identity"),method="GCV.Cp",select=TRUE)
plot(vc.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc.shrink)[1])
ggGam(vc.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TP, TOC, WT, EC, TC, NH3N, PO4P, Rain
summary(vc.shrink) # R-sq.(adj) =  0.727   Deviance explained = 75.2%
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
```{r}
pred.tvcm <- predict(vc.shrink,newdata=ex1_scale,type="response")
data.tvcm <- data.frame(response=ex1_scale$Chla,fitted_values=pred.tvcm)
ggplot(data.tvcm, aes(x=fitted_values, y=response)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Time Varying Coefficient Model)")
```
...
#### 최종선택된 모형의 설명력 : 65.6%(다중선형회귀모형), 65.6%(일반화선형모형), 73.3%(일반화가법모형), 75.2%(시간에 따른 계수변화모형) 
...
plot(mm,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm)[1])
ggGam(mm)
summary(mm) # R-sq.(adj) =  0.712   Deviance explained = 76.9%
concurvity(mm,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
plot(mm.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm.shrink)[1])
ggGam(mm.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TN, TP, TOC, WT, EC, Chla, NH3N, PO4P, FC, Flow, Rain
summary(mm.shrink) # R-sq.(adj) =  0.712   Deviance explained = 76.8%
concurvity(mm.shrink,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm.shrink) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
plot(vc,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc)[1],ylim=c(-50,50))
ggGam(vc)
summary(vc) # R-sq.(adj) =  0.813   Deviance explained = 81.3%
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
plot(vc.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc.shrink)[1],ylim=c(-50,50))
ggGam(vc.shrink) # 선택변수 : DO, COD, SS, TN, TP, TOC, WT, EC, NH3N, PO4P, FC, Flow, Rain
summary(vc.shrink) # R-sq.(adj) =   0.81   Deviance explained = 80.8% 
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
#### 최종선택된 모형의 설명력 : 48.9%(다중선형회귀모형), 42.2%(일반화선형모형), 76.8%(일반화가법모형), 80.8%(시간에 따른 계수변화모형) 
...
plot(mm,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm)[1])
ggGam(mm)
summary(mm) # R-sq.(adj) =  0.948   Deviance explained = 88.8%
concurvity(mm,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
summary(mm.shrink) # R-sq.(adj) =  0.949   Deviance explained = 88.7%
concurvity(mm.shrink,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm.shrink) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
plot(vc,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc)[1],ylim=c(-50,50))
ggGam(vc)
summary(vc) # R-sq.(adj) =  0.959   Deviance explained = 90.7%
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
summary(vc.shrink) # R-sq.(adj) =  0.959   Deviance explained = 90.7%
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
#### 최종선택된 모형의 설명력 : 40.3%(다중선형회귀모형), 60.6%(일반화선형모형), 88.7%(일반화가법모형), 90.7%(시간에 따른 계수변화모형)  
...
```{r}
pred.mlr <- predict(gfit.step,newdata=ex2_scale,type="response")
data.mlr <- data.frame(response=ex2_scale$Chla,fitted_values=pred.mlr)
ggplot(data.mlr, aes(x=fitted_values, y=response)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Multiple Linear Regression)")
```
...
```{r}
m <- glm(Chla~pH+DO+BOD+COD+SS+TN+TP+TOC+WT+EC+FC+NH3N+PO4P+TC+Flow+Rain,data=ex2_scale,family=gaussian(link="identity"))
summary(m) # Deviance explained : 0.7130
vif(m) # 다중공선성(multicollinearity) 존재
par(mfrow=c(2,2))
plot(m)
par(mfrow=c(1,1))
...
```{r}
m.step <- stepAIC(m, direction="both", trace=TRUE)
m.step # 선택변수 : pH, DO, BOD, COD, TN, TP, WT, EC, NH3N, PO4P, Rain
summary(m.step) # Deviance explained : 0.7117
vif(m.step) # 다중공선성(multicollinearity) 존재 
par(mfrow=c(2,2))
plot(m.step)
par(mfrow=c(1,1))
...
```{r}
pred.glm <- predict(m.step,newdata=ex2_scale,type="response")
data.glm <- data.frame(response=ex2_scale$Chla,fitted_values=pred.glm)
ggplot(data.glm, aes(x=fitted_values, y=response)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Linear Model)")
```
...
```{r}
# GAM의 경우 추정되는 계수(coefficients)가 데이터의 개수보다 많으면 적합되지 않음.
mm <- gam(Chla~s(pH)+s(DO)+s(BOD)+s(COD)+s(SS)+s(TN)+s(TP)+s(TOC)+s(WT)+s(EC)+s(TC)+s(NH3N)+s(PO4P)+s(FC)+s(Flow)+s(Rain),data=ex2_scale,family=gaussian(link="identity"))
plot(mm,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm)[1])
ggGam(mm)
summary(mm) # R-sq.(adj) =  0.753   Deviance explained = 77.2%
concurvity(mm,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
```{r}
mm.shrink <- gam(Chla~s(pH)+s(DO)+s(BOD)+s(COD)+s(SS)+s(TN)+s(TP)+s(TOC)+s(WT)+s(EC)+s(TC)+s(NH3N)+s(PO4P)+s(FC)+s(Flow)+s(Rain),data=ex2_scale,family=gaussian(link="identity"),method="GCV.Cp",select=TRUE) 
plot(mm.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm.shrink)[1])
ggGam(mm.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TN, TP, TOC, WT, EC, TC, NH3N, PO4P
summary(mm.shrink) # R-sq.(adj) =  0.754   Deviance explained = 77.1%
concurvity(mm.shrink,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm.shrink) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
```{r}
pred.gam <- predict(mm.shrink,newdata=ex2_scale,type="response")
data.gam <- data.frame(response=ex2_scale$Chla,fitted_values=pred.gam)
ggplot(data.gam, aes(x=fitted_values, y=response)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Generalized Additive Model)")
```
...
head(ex2_scale)
vc <- gam(Chla~s(time)+s(time,by=pH)+s(time,by=DO)+s(time,by=BOD)+s(time,by=COD)+s(time,by=SS)+s(time,by=TN)+s(time,by=TP)+s(time,by=TOC)+s(time,by=WT)+s(time,by=EC)+s(time,by=TC)+s(time,by=NH3N)+s(time,by=PO4P)+s(time,by=FC)+s(time,by=Flow)+s(time,by=Rain),data=ex2_scale,family=gaussian(link="identity"))
plot(vc,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc)[1])
ggGam(vc)
summary(vc) # R-sq.(adj) =  0.789   Deviance explained = 81.6%
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
```{r}
vc.shrink <- gam(Chla~s(time)+s(time,by=pH)+s(time,by=DO)+s(time,by=BOD)+s(time,by=COD)+s(time,by=SS)+s(time,by=TN)+s(time,by=TP)+s(time,by=TOC)+s(time,by=WT)+s(time,by=EC)+s(time,by=TC)+s(time,by=NH3N)+s(time,by=PO4P)+s(time,by=FC)+s(time,by=Flow)+s(time,by=Rain),data=ex2_scale,family=gaussian(link="identity"),method="GCV.Cp",select=TRUE)
plot(vc.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc.shrink)[1])
ggGam(vc.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TN, TP, WT, EC, NH3N, PO4P, FC, Rain
summary(vc.shrink) # R-sq.(adj) =  0.793   Deviance explained = 81.4%
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
```{r}
pred.tvcm <- predict(vc.shrink,newdata=ex2_scale,type="response")
data.tvcm <- data.frame(response=ex2_scale$Chla,fitted_values=pred.tvcm)
ggplot(data.tvcm, aes(x=fitted_values, y=response)) +
  geom_point() + geom_rug() +
  ggtitle("response vs fitted_values (Time Varying Coefficient Model)")
```
...
#### 최종선택된 모형의 설명력 : 70.5%(다중선형회귀모형), 71.2%(일반화선형모형), 77.1%(일반화가법모형), 81.4%(시간에 따른 계수변화모형) 
