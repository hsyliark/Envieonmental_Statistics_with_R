...
plot(vc,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc)[1],ylim=c(-40,40))
ggGam(vc)
summary(vc) # Deviance explained = 77.3%    
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
```{r}
vc.shrink <- gam(Chla~s(time)+s(time,by=pH)+s(time,by=DO)+s(time,by=BOD)+s(time,by=COD)+s(time,by=SS)+s(time,by=TN)+s(time,by=TP)+s(time,by=TOC)+s(time,by=WT)+s(time,by=EC)+s(time,by=log(TC))+s(time,by=NH3N)+s(time,by=PO4P)+s(time,by=log(FC))+s(time,by=Flow)+s(time,by=Rain),data=ex1,family=quasi(link="log"),method="REML",select=TRUE)
plot(vc.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc.shrink)[1],ylim=c(-40,40))
ggGam(vc.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TP, TOC, WT, log(TC), NH3N, PO4P, log(FC), Flow, Rain 
summary(vc.shrink) # Deviance explained = 74.6%    
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
pred.tvcm <- predict(vc.shrink,newdata=ex1,type="response")
data.tvcm <- data.frame(response=ex1$Chla,fitted_values=pred.tvcm,time=ex1$time)
sqrt(sum((data.tvcm$response-data.tvcm$fitted_values)^2)/length(data.tvcm$response)) # RMSE = 16.86484     
ggplot(data.tvcm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
...
pred.tvcm1 <- predict(vc.shrink,newdata=ex1_1,type="response")
data.tvcm1 <- data.frame(response=ex1_1$Chla,fitted_values=pred.tvcm1,time=ex1_1$time)
sqrt(sum((data.tvcm1$response-data.tvcm1$fitted_values)^2)/length(data.tvcm1$response)) # RMSE = 42.18933    
ggplot(data.tvcm1, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
...
#### 최종선택된 모형의 설명력(Deviance explained) : 64.8%(다중선형회귀모형), 59.7%(일반화선형모형(Gamma)), 69.3%(일반화가법모형(Gamma)), 75.7%(일반화가법모형(quasi)), 69.9%(시간에 따른 계수변화모형(Gamma)), 74.6%(시간에 따른 계수변화모형(quasi)) 

#### train data 관련 최종선택된 모형의 예측력(Root Mean Squared Error) : 19.85979(다중선형회귀모형), 31.34176(일반화선형모형(Gamma)), 20.33466(일반화가법모형(Gamma)), 16.48926(일반화가법모형(quasi)), 27.81185(시간에 따른 계수변화모형(Gamma)), 16.86484(시간에 따른 계수변화모형(quasi))

#### test data 관련 최종선택된 모형의 예측력(Root Mean Squared Error) : 39.45355(다중선형회귀모형), 46.81785(일반화선형모형(Gamma)), 39.9079(일반화가법모형(Gamma)), 37.7436(일반화가법모형(quasi)), 46.76845(시간에 따른 계수변화모형(Gamma)), 42.18933(시간에 따른 계수변화모형(quasi))
...
fit <- glm(Chla~pH+DO+BOD+COD+SS+TN+TP+TOC+WT+EC+log(FC)+NH3N+PO4P+log(TC)+Flow+Rain,data=ex2,family=gaussian(link="identity"))
summary(fit) # Deviance explained: 0.7128    
vif(fit) 
par(mfrow=c(2,2))
plot(fit)
par(mfrow=c(1,1))
...
```{r}
fit.step <- stepAIC(fit, direction="both", trace=TRUE)
fit.step # 선택변수 : pH, DO, BOD, COD, TN, TP, WT, EC, NH3N, PO4P, Rain 
summary(fit.step) # Deviance explained: 0.7117    
vif(fit.step)  
par(mfrow=c(2,2))
plot(fit.step)
par(mfrow=c(1,1))
...
sqrt(sum((data.mlr$response-data.mlr$fitted_values)^2)/
       length(data.mlr$response)) # RMSE = 12.63798  
ggplot(data.mlr, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
...
sqrt(sum((data.mlr1$response-data.mlr1$fitted_values)^2)/
       length(data.mlr1$response)) # RMSE = 15.08679    
ggplot(data.mlr1, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
...
summary(m) # Deviance explained : 0.6918  
vif(m) 
par(mfrow=c(2,2))
plot(m)
par(mfrow=c(1,1))
...
sqrt(sum((data.glm$response-data.glm$fitted_values)^2)/
       length(data.glm$response)) # RMSE = 22.85586    
ggplot(data.glm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla train data] response vs fitted_values 
...
sqrt(sum((data.glm1$response-data.glm1$fitted_values)^2)/
       length(data.glm1$response)) # RMSE = 15.70857  
ggplot(data.glm1, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla test data] response vs fitted_values 
          (Generalized Linear Model, (Gamma Distribution))") +
...
```{r}
# GAM의 경우 추정되는 계수(coefficients)가 데이터의 개수보다 많으면 적합되지 않음.
mm <- gam(Chla~s(pH)+s(DO)+s(BOD)+s(COD)+s(SS)+s(TN)+s(TP)+s(TOC)+s(WT)+s(EC)+s(log(TC))+s(NH3N)+s(PO4P)+s(log(FC))+s(Flow)+s(Rain),data=ex2,family=Gamma(link="log"),method="REML")
plot(mm,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm)[1])
ggGam(mm)
summary(mm) # Deviance explained = 78.9%  
concurvity(mm,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
```{r}
mm.shrink <- gam(Chla~s(pH)+s(DO)+s(BOD)+s(COD)+s(SS)+s(TN)+s(TP)+s(TOC)+s(WT)+s(EC)+s(log(TC))+s(NH3N)+s(PO4P)+s(log(FC))+s(Flow)+s(Rain),data=ex2,family=Gamma(link="log"),method="REML",select=TRUE) 
plot(mm.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm.shrink)[1])
ggGam(mm.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TN, TP, TOC, WT, EC, NH3N, PO4P, log(FC), Flow, Rain   
...
sqrt(sum((data.gam$response-data.gam$fitted_values)^2)/
       length(data.gam$response)) # RMSE = 11.38836     
ggplot(data.gam, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla train data] response vs fitted_values 
...
sqrt(sum((data.gam1$response-data.gam1$fitted_values)^2)/
       length(data.gam1$response)) # RMSE = 13.8368   
ggplot(data.gam1, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla test data] response vs fitted_values 
          (Generalized Additive Model, (Gamma Distribution))") +
...
```{r}
# GAM의 경우 추정되는 계수(coefficients)가 데이터의 개수보다 많으면 적합되지 않음.
mm <- gam(Chla~s(pH)+s(DO)+s(BOD)+s(COD)+s(SS)+s(TN)+s(TP)+s(TOC)+s(WT)+s(EC)+s(log(TC))+s(NH3N)+s(PO4P)+s(log(FC))+s(Flow)+s(Rain),data=ex2,family=quasi(link="log"),method="REML")
plot(mm,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm)[1])
ggGam(mm)
summary(mm) # Deviance explained = 86.3%  
concurvity(mm,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
```{r}
mm.shrink <- gam(Chla~s(pH)+s(DO)+s(BOD)+s(COD)+s(SS)+s(TN)+s(TP)+s(TOC)+s(WT)+s(EC)+s(log(TC))+s(NH3N)+s(PO4P)+s(log(FC))+s(Flow)+s(Rain),data=ex2,family=quasi(link="log"),method="REML",select=TRUE) 
plot(mm.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm.shrink)[1])
ggGam(mm.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TN, TP, TOC, WT, EC, NH3N, PO4P, log(FC), Flow   
summary(mm.shrink) # Deviance explained = 85.7%    
concurvity(mm.shrink,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm.shrink) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
data.gam <- data.frame(response=ex2$Chla,fitted_values=pred.gam,
                       time=ex2$time)
sqrt(sum((data.gam$response-data.gam$fitted_values)^2)/
       length(data.gam$response)) # RMSE = 8.900784     
ggplot(data.gam, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla train data] response vs fitted_values 
...
data.gam1 <- data.frame(response=ex2_1$Chla,fitted_values=pred.gam1,
                        time=ex2_1$time)
sqrt(sum((data.gam1$response-data.gam1$fitted_values)^2)/
       length(data.gam1$response)) # RMSE = 21.57132   
ggplot(data.gam1, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla test data] response vs fitted_values 
          (Generalized Additive Model, (Quasi Distribution))") +
...
```{r}
vc <- gam(Chla~s(time)+s(time,by=pH)+s(time,by=DO)+s(time,by=BOD)+s(time,by=COD)+s(time,by=SS)+s(time,by=TN)+s(time,by=TP)+s(time,by=TOC)+s(time,by=WT)+s(time,by=EC)+s(time,by=log(TC))+s(time,by=NH3N)+s(time,by=PO4P)+s(time,by=log(FC))+s(time,by=Flow)+s(time,by=Rain),data=ex2,family=Gamma(link="log"),method="REML")
plot(vc,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc)[1],ylim=c(-40,40))
ggGam(vc)
summary(vc) # Deviance explained = 77.3%     
par(mfrow=c(2,2))
...
```{r}
vc.shrink <- gam(Chla~s(time)+s(time,by=pH)+s(time,by=DO)+s(time,by=BOD)+s(time,by=COD)+s(time,by=SS)+s(time,by=TN)+s(time,by=TP)+s(time,by=TOC)+s(time,by=WT)+s(time,by=EC)+s(time,by=log(TC))+s(time,by=NH3N)+s(time,by=PO4P)+s(time,by=log(FC))+s(time,by=Flow)+s(time,by=Rain),data=ex2,family=Gamma(link="log"),method="REML",select=TRUE)
plot(vc.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc.shrink)[1],ylim=c(-40,40))
ggGam(vc.shrink) # 선택변수 : DO, BOD, COD, SS, TN, TP, TOC, EC, NH3N, PO4P, log(FC), Flow, Rain 
summary(vc.shrink) # Deviance explained = 76%    
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
sqrt(sum((data.tvcm$response-data.tvcm$fitted_values)^2)/
       length(data.tvcm$response)) # RMSE = 18.6995     
ggplot(data.tvcm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla train data] response vs fitted_values 
...
sqrt(sum((data.tvcm1$response-data.tvcm1$fitted_values)^2)/
       length(data.tvcm1$response)) # RMSE = 39.34194   
ggplot(data.tvcm1, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla test data] response vs fitted_values 
          (Time Varying Coefficient Model, (Gamma Distribution))") +
...
```{r}
vc <- gam(Chla~s(time)+s(time,by=pH)+s(time,by=DO)+s(time,by=BOD)+s(time,by=COD)+s(time,by=SS)+s(time,by=TN)+s(time,by=TP)+s(time,by=TOC)+s(time,by=WT)+s(time,by=EC)+s(time,by=log(TC))+s(time,by=NH3N)+s(time,by=PO4P)+s(time,by=log(FC))+s(time,by=Flow)+s(time,by=Rain),data=ex2,family=quasi(link="log"),method="REML")
plot(vc,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc)[1],ylim=c(-40,40))
ggGam(vc)
summary(vc) # Deviance explained = 87%     
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
```{r}
vc.shrink <- gam(Chla~s(time)+s(time,by=pH)+s(time,by=DO)+s(time,by=BOD)+s(time,by=COD)+s(time,by=SS)+s(time,by=TN)+s(time,by=TP)+s(time,by=TOC)+s(time,by=WT)+s(time,by=EC)+s(time,by=log(TC))+s(time,by=NH3N)+s(time,by=PO4P)+s(time,by=log(FC))+s(time,by=Flow)+s(time,by=Rain),data=ex2,family=quasi(link="log"),method="REML",select=TRUE)
plot(vc.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc.shrink)[1],ylim=c(-40,40))
ggGam(vc.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TN, TP, TOC, EC, log(TC), NH3N, PO4P, Flow 
summary(vc.shrink) # Deviance explained = 83.7%     
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
data.tvcm <- data.frame(response=ex2$Chla,fitted_values=pred.tvcm,
                        time=ex2$time)
sqrt(sum((data.tvcm$response-data.tvcm$fitted_values)^2)/
       length(data.tvcm$response)) # RMSE = 9.500878     
ggplot(data.tvcm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla train data] response vs fitted_values 
...
data.tvcm1 <- data.frame(response=ex2_1$Chla,fitted_values=pred.tvcm1,
                         time=ex2_1$time)
sqrt(sum((data.tvcm1$response-data.tvcm1$fitted_values)^2)/
       length(data.tvcm1$response)) # RMSE = 17.87177  
ggplot(data.tvcm1, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla test data] response vs fitted_values 
          (Time Varying Coefficient Model, (Quasi Distribution))") +
...
#### 최종선택된 모형의 설명력(Deviance explained) : 71.2%(다중선형회귀모형), 69.0%(일반화선형모형(Gamma)), 78.0%(일반화가법모형(Gamma)), 85.7%(일반화가법모형(quasi)), 76.0%(시간에 따른 계수변화모형(Gamma)), 83.7%(시간에 따른 계수변화모형(quasi))

#### train data 관련 최종선택된 모형의 예측력(Root Mean Squared Error) : 12.63798(다중선형회귀모형), 22.85586(일반화선형모형(Gamma)), 11.38836(일반화가법모형(Gamma)), 8.900784(일반화가법모형(quasi)), 18.6995(시간에 따른 계수변화모형(Gamma)), 9.500878(시간에 따른 계수변화모형(quasi))

#### test data 관련 최종선택된 모형의 예측력(Root Mean Squared Error) : 15.08679(다중선형회귀모형), 15.70857(일반화선형모형(Gamma)), 13.8368(일반화가법모형(Gamma)), 21.57132(일반화가법모형(quasi)), 39.34194(시간에 따른 계수변화모형(Gamma)), 17.87177(시간에 따른 계수변화모형(quasi))
...
#### 여러가지 사항을 고려했을 때 우치 Chla 예측에 대한 최적의 모형은 GAM(Gamma), GAM(quasi)라고 판단됨.