# 반응변수 la
# ???�설명변수 : SS, TN, TP, TOC, log(TC), Flow, Rain

ex1 <- read.csv("C:/Users/stat/Desktop/????(2010-광산).csv", sep=",", header=T)
ex1 <- ex1[,-1]
ex1 <- as.data.frame(ex1)

ex2 <- read.csv("C:/Users/stat/Desktop/??ġ(2010-우치(2010-2019).csv", sep=",", header=T)
ex2 <- ex2[,-1].data.frame(ex2)
