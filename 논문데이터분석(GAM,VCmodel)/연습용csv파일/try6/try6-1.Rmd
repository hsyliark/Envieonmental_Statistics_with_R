...
ggGam(mm.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TN, TP, TOC, WT, EC, log(TC), NH3N, PO4P, log(FC), Flow, Rain 
summary(mm.shrink) # Deviance explained = 72.3%   
...
sqrt(sum((data.gam$response-data.gam$fitted_values)^2)/length(data.gam$response)) # RMSE = 19.23435      
...
sqrt(sum((data.gam1$response-data.gam1$fitted_values)^2)/length(data.gam1$response)) # RMSE = 38.33993    
...
ggGam(vc)
summary(vc) # Deviance explained = 75.5%    
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
ggGam(vc.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TP, TOC, WT, EC, PO4P, log(FC), Flow, Rain 
summary(vc.shrink) # Deviance explained = 71.5%   
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
sqrt(sum((data.tvcm$response-data.tvcm$fitted_values)^2)/length(data.tvcm$response)) # RMSE = 25.17242     
...
sqrt(sum((data.tvcm1$response-data.tvcm1$fitted_values)^2)/length(data.tvcm1$response)) # RMSE = 45.5266    
...
#### 최종선택된 모형의 설명력(Deviance explained) : 64.8%(다중선형회귀모형), 59.7%(일반화선형모형(Gamma)), 72.3%(일반화가법모형(Gamma)), 76.9%(일반화가법모형(quasi)), 71.5%(시간에 따른 계수변화모형(Gamma)), 80.3%(시간에 따른 계수변화모형(quasi)) 

#### train data 관련 최종선택된 모형의 예측력(Root Mean Squared Error) : 19.85979(다중선형회귀모형), 31.34176(일반화선형모형(Gamma)), 19.23435(일반화가법모형(Gamma)), 16.08552(일반화가법모형(quasi)), 25.17242(일반화가법모형(Gamma)), 14.85056(시간에 따른 계수변화모형(quasi))

#### test data 관련 최종선택된 모형의 예측력(Root Mean Squared Error) : 39.45355(다중선형회귀모형), 46.81785(일반화선형모형(Gamma)), 38.33993(일반화가법모형(Gamma)), 37.13758(일반화가법모형(quasi)), 45.5266(시간에 따른 계수변화모형(Gamma)), 48.43549(시간에 따른 계수변화모형(quasi))
...
plot(mm,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm)[1])
ggGam(mm)
summary(mm) # Deviance explained = 79.8%  
concurvity(mm,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
sqrt(sum((data.gam$response-data.gam$fitted_values)^2)/
       length(data.gam$response)) # RMSE = 10.93831     
ggplot(data.gam, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla train data] response vs fitted_values 
...
sqrt(sum((data.gam1$response-data.gam1$fitted_values)^2)/
       length(data.gam1$response)) # RMSE = 12.87   
ggplot(data.gam1, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla test data] response vs fitted_values 
          (Generalized Additive Model, (Gamma Distribution))") +
...
plot(mm.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(mm.shrink)[1])
ggGam(mm.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TN, TP, TOC, WT, EC, log(TC), NH3N, PO4P, log(FC), Rain  
summary(mm.shrink) # Deviance explained = 88.8%   
concurvity(mm.shrink,full=TRUE) # 공선성 존재
par(mfrow=c(2,2))
gam.check(mm.shrink) # 적합결여 존재 (기저함수 부족)
par(mfrow=c(1,1))
...
data.gam <- data.frame(response=ex2$Chla,fitted_values=pred.gam,
                       time=ex2$time)
sqrt(sum((data.gam$response-data.gam$fitted_values)^2)/
       length(data.gam$response)) # RMSE = 7.873835     
ggplot(data.gam, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla train data] response vs fitted_values 
...
data.gam1 <- data.frame(response=ex2_1$Chla,fitted_values=pred.gam1,
                        time=ex2_1$time)
sqrt(sum((data.gam1$response-data.gam1$fitted_values)^2)/
       length(data.gam1$response)) # RMSE = 22.68404   
ggplot(data.gam1, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla test data] response vs fitted_values 
          (Generalized Additive Model, (Quasi Distribution))") +
...
plot(vc,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc)[1],ylim=c(-40,40))
ggGam(vc)
summary(vc) # Deviance explained = 80.8%    
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
summary(vc.shrink) # Deviance explained = 79.5%    
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
sqrt(sum((data.tvcm$response-data.tvcm$fitted_values)^2)/
       length(data.tvcm$response)) # RMSE = 15.69044     
ggplot(data.tvcm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla train data] response vs fitted_values 
...
sqrt(sum((data.tvcm1$response-data.tvcm1$fitted_values)^2)/
       length(data.tvcm1$response)) # RMSE = 40.33535   
ggplot(data.tvcm1, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla test data] response vs fitted_values 
          (Time Varying Coefficient Model, (Gamma Distribution))") +
...
plot(vc,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc)[1],ylim=c(-40,40))
ggGam(vc)
summary(vc) # Deviance explained = 91.9%    
par(mfrow=c(2,2))
gam.check(vc) 
par(mfrow=c(1,1))
```
...
plot(vc.shrink,residuals=TRUE,pch=1,shade=TRUE,seWithMean=TRUE,shift=coef(vc.shrink)[1],ylim=c(-40,40))
ggGam(vc.shrink) # 선택변수 : pH, DO, BOD, COD, SS, TN, TP, TOC, WT, EC, log(TC), NH3N, PO4P, log(FC), Flow 
summary(vc.shrink) # Deviance explained = 90%    
par(mfrow=c(2,2))
gam.check(vc.shrink) 
par(mfrow=c(1,1))
```
...
data.tvcm <- data.frame(response=ex2$Chla,fitted_values=pred.tvcm,
                        time=ex2$time)
sqrt(sum((data.tvcm$response-data.tvcm$fitted_values)^2)/
       length(data.tvcm$response)) # RMSE = 7.427525     
ggplot(data.tvcm, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla train data] response vs fitted_values 
...
data.tvcm1 <- data.frame(response=ex2_1$Chla,fitted_values=pred.tvcm1,
                         time=ex2_1$time)
sqrt(sum((data.tvcm1$response-data.tvcm1$fitted_values)^2)/
       length(data.tvcm1$response)) # RMSE = 18.35658  
ggplot(data.tvcm1, aes(x=response, y=fitted_values)) +
  geom_point() + geom_rug() +
  ggtitle("[Uchi Chla test data] response vs fitted_values 
          (Time Varying Coefficient Model, (Quasi Distribution))") +
...
#### 최종선택된 모형의 설명력(Deviance explained) : 71.2%(다중선형회귀모형), 69.0%(일반화선형모형(Gamma)), 79.6%(일반화가법모형(Gamma)), 88.8%(일반화가법모형(quasi)), 79.5%(시간에 따른 계수변화모형(Gamma)), 90.0%(시간에 따른 계수변화모형(quasi))

#### train data 관련 최종선택된 모형의 예측력(Root Mean Squared Error) : 12.63798(다중선형회귀모형), 22.85586(일반화선형모형(Gamma)), 10.93831(일반화가법모형(Gamma)), 7.873835(일반화가법모형(quasi)), 15.69044(시간에 따른 계수변화모형(Gamma)), 7.427525(시간에 따른 계수변화모형(quasi))

#### test data 관련 최종선택된 모형의 예측력(Root Mean Squared Error) : 15.08679(다중선형회귀모형), 15.70857(일반화선형모형(Gamma)), 12.87(일반화가법모형(Gamma)), 22.68404(일반화가법모형(quasi)), 40.33535(시간에 따른 계수변화모형(Gamma)), 18.35658(시간에 따른 계수변화모형(quasi))
