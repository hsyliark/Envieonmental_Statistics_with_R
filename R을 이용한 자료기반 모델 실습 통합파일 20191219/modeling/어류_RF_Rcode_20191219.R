...
setwd("C:/Users/3층 전산강의실/Desktop/modeling")

## 풍부도자료(Abundance): 개체수
sp.DB <- read.csv("수생태_어류_생물.csv", stringsAsFactors=F, sep=',')
str(sp.DB)

## 환경 자료
Env.DB <- read.csv("수생태_어류_환경.csv", stringsAsFactors=F, sep=',')
str(Env.DB)
...
#형성된 자료의 열을 기준으로 생성
target.sp = 1 # data에서 종 선택, column . ex) 1: 피라미, 2: 참갈겨니...
...
#####################################################################
################################### 3. 자료의 전처리 과정
targetDB <- Total.DB # environmental variavles  sites x no_variables
head(targetDB)
...
plot(m4[,4],m5[,4]) # scale 결과 점검

#6. 환경 변수 정보(var.info) 저장
target.env.DB <- m4[,2:(dim(m4)[2])] # scale 하기 바로 직전 DB
total.var.name <- names(target.env.DB)  # number of variables
...
### 1) AUC
RF_p1 <- ROCR::prediction(as.numeric(as.vector(RF.test)), RF.test_DB$pop)
RF_pp1 <- performance(RF_p1, 'tpr', 'fpr')
RF_auc1 <- performance(RF_p1, "auc")
...
#
partialPlot(RF.model, train_DB, PDP.variables[1], plot=T, main="", xlab=PDP.variables[1])
...
pred.result <- data.frame("서식확률"=RF.pred)
